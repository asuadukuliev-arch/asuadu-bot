...
SYSTEM_PROMPT = """Ты — помощник конного клуба ASUADU (Нальчик). Отвечаешь клиентам в Telegram от имени клуба.
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Сообщение от {user_name}: {user_message}")
    ...
    try:
        ...
        logger.info(f"Ответ отправлен: {reply[:50]}")
